        ...
        case .none, .text:
            if let _ = chatPresentationInterfaceState.interfaceState.editMessage {
                ...
                if chatPresentationInterfaceState.interfaceState.composeInputState.inputText.length == 0 && chatPresentationInterfaceState.interfaceState.forwardMessageIds == nil {
                    ...
                    // MARK: - iMe
                    var isBotChat = (chatPresentationInterfaceState.renderedPeer?.peer as? TelegramUser)?.botInfo != nil
                    isBotChat = isBotChat || chatPresentationInterfaceState.hasBots

                    let isChannelChat: Bool
                    if case .broadcast? = (chatPresentationInterfaceState.renderedPeer?.peer as? TelegramChannel)?.info {
                        isChannelChat = true
                    } else {
                        isChannelChat = false
                    }

                    if !isBotChat && !isChannelChat {
