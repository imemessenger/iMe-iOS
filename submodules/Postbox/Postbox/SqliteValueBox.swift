...
public final class SqliteValueBox: ValueBox {
    ...
    // MARK: - iMe
    // Without this fix an app will crash during a first lounch
    private func createTable(database: Database, table: ValueBoxTable) {
        ...
        switch table.keyType {
            ...
            case .int64:
                resultCode = database.execute("CREATE TABLE t\(table.id) (key INTEGER PRIMARY KEY, value BLOB)")
        }
        ...
        if !resultCode {
            ...
            precondition(tables.index(forKey: table.id) != nil)
        }
    }
    // MARK: -
