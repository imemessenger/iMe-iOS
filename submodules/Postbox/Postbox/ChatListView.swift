...
final class MutableChatListView {
    ...
    // MARK: - iMe
    var chatListMode: PostBoxChatListMode = .standard
...
public final class ChatListView {
    ...
    // MARK: -
        ...
        let filteredEntries = mutableView.applyFiltration
            ? chatListHandler(mutableView.entries, true, mutableView.chatListMode)
            : mutableView.entries
        ...
        self.groupEntries = mutableView.chatListMode.isFolders ? [] : mutableView.groupEntries
        // MARK: -
        ...
        let filteredAdditionalEntries = mutableView.applyFiltration
            ? chatListHandler(mutableView.additionalItemEntries, false, mutableView.chatListMode)
            : mutableView.additionalItemEntries
        
        for entry in filteredAdditionalEntries {
            switch entry {
                case let .MessageEntry(index, message, combinedReadState, notificationSettings, embeddedState, peer, peerPresence, summaryInfo):
