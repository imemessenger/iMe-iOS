...
public func openPostbox(basePath: String, seedConfiguration: SeedConfiguration, encryptionParameters: ValueBoxEncryptionParameters, isIncluded: @escaping IsIncludedClosure = { _, _ in true }) -> Signal<PostboxResult, NoError> {
    ...
    return Signal { subscriber in
        queue.async {
            ...
            loop: while true {
                ...
                subscriber.putNext(.postbox(Postbox(queue: queue, basePath: basePath, seedConfiguration: seedConfiguration, valueBox: valueBox, isIncluded: isIncluded)))
...
public final class Postbox {
    ...
    // MARK: - iMe
    init(queue: Queue, basePath: String, seedConfiguration: SeedConfiguration, valueBox: SqliteValueBox, isIncluded: @escaping IsIncludedClosure) {
    ...
    // MARK: -
        return self.aroundChatListView(groupId: groupId, index: ChatListIndex.absoluteUpperBound, count: count, summaryComponents: summaryComponents, userInteractive: true, applyFiltration: applyFiltration, setupChatListModeHandler: setupChatListModeHandler)
    }
    
    // MARK: - iMe
    public func aroundChatListView(groupId: PeerGroupId, index: ChatListIndex, count: Int, summaryComponents: ChatListEntrySummaryComponents, userInteractive: Bool = false, applyFiltration: Bool = false, setupChatListModeHandler: SetupChatListModeCallback? = nil) -> Signal<(ChatListView, ViewUpdateType), NoError> {
    // MARK: -
    ...
    private lazy var chatListHandler: ChatListFilterClosure = { [weak self] entries, applyFiltration, chatListMode in
        switch chatListMode {
        case .standard:
            return entries
        case .unread:
            return entries.filter {
                guard case let .MessageEntry(_, _, readState, _, _, _, _, _) = $0 else {
                    return false
                }
        ...
        case let .folders(folders):
            guard applyFiltration else { return [] }
            return folders
                .lazy
                ...
                .compactMap {
                    ...
                    return .MessageEntry(chatListIndex, $0.lastMessage, combinedReadState, nil, nil, renderedPeer, nil, .init())
                }
        }
    }
}
...
// MARK: - iMe Getting unread categories
...
private func getUnreadCategories(
    ...
    for entry in entries {
        if case let .MessageEntry(_, _, readState, _, _, renderedPeer, _, _) = entry {
            guard
...
extension Postbox {
    ...
    public func readAllIncomingMessages() {
        readAllIncomingMessagesDisposable?.dispose()
        readAllIncomingMessagesDisposable = (tailChatListView(groupId: .root, count: 0, summaryComponents: .init(), applyFiltration: false, setupChatListModeHandler: nil)
            |> take(1)
            |> map { (chatListView, _) -> [MessageIndex] in
                return chatListView.entries.compactMap {
                    guard
                        case let .MessageEntry(index, _, readState?, _, _, _, _, _) = $0,
                        readState.isUnread
                    else { return nil }
    ...
    private func watchChatListUpdates() {
        incomingMessagesDisposable = self.tailChatListView(groupId: .root, count: 0, summaryComponents: .init())
            .start(next: { [weak folderManager] chatListView, update in
                ...
                if case .Generic = update {
                    chatListView.entries.forEach {
                        guard
                            case let .MessageEntry(_, message, readState, _, _, renderedPeer, _, _) = $0,
                            let peer = renderedPeer.peer
                        else { return }
    ...
    private func watchUnreadCategories() {
        unreadCategoriesDisposable = self.tailChatListView(groupId: .root, count: 0, summaryComponents: .init())
