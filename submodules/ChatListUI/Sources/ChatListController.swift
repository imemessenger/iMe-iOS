...
public typealias ViewControllerProvider = (AccountContext) -> ViewController
// MARK: -
...
public class ChatListControllerImpl: TelegramBaseController, ChatListController, UIViewControllerPreviewingDelegate {
    ...
    // MARK: - iMe
    private var channelControllerProvider: ViewControllerProvider?
    ...
    var chatListMode: ChatListMode {
        didSet {
            chatListDisplayNode.chatListNode.snapToTop()
            chatListModeSwitcher?(chatListMode)
    ...
    private var placeholderType: PlaceholderType {
        switch chatListMode {
            case .folders: return .folders
            case .unread: return .unread
            case .filter(.channels): return .channels
    ...
    public init(
        ...
        enableDebugActions: Bool,
        channelControllerProvider: ViewControllerProvider? = nil
    ) {
    // MARK: -
        ...
        // MARK: - iMe
        self.chatListMode = groupId == .root ? .unread : .standard
        self.channelControllerProvider = channelControllerProvider
        // MARK: -
        ...
        if case .root = groupId {
            ...
            // MARK: - iMe
            let (buttonIcon, accessebilityTitle) = getRightBarButtonIconAndAccessebilityTitle()
            let rightBarButtonItem = UIBarButtonItem(image: buttonIcon, style: .plain, target: self, action: #selector(self.composePressed))
            rightBarButtonItem.accessibilityLabel = accessebilityTitle
                ...
                if let strongSelf = self {
                    ...
                    if state.editing {
                        ...
                        if case .root = strongSelf.groupId {
                            ...
                            // MARK: - iMe
                            let (buttonIcon, accessebilityTitle) = strongSelf.getRightBarButtonIconAndAccessebilityTitle()
                            let rightBarButtonItem = UIBarButtonItem(image: buttonIcon, style: .plain, target: strongSelf, action: #selector(strongSelf.composePressed))
                            rightBarButtonItem.accessibilityLabel = accessebilityTitle
                            // MARK: -
    ...
    private func updateThemeAndStrings() {
        ...
        // MARK: - iMe
        self.chatListDisplayNode.tabBarView?.theme = self.presentationData.theme
        // MARK: -
    ...
    override public func loadDisplayNode() {
        // MARK: - iMe
        let showsFilterTabs = groupId == .root
        self.displayNode = ChatListControllerNode(
            context: self.context,
            groupId: self.groupId,
            controlsHistoryPreload: self.controlsHistoryPreload,
            presentationData: self.presentationData,
            controller: self,
            showsFilterTabs: showsFilterTabs,
            placeholderType: self.placeholderType,
                    ...
                    actionSheet.setItemGroups([
                        ActionSheetItemGroup(items: [
                            ActionSheetButtonItem(
                                title: self.presentationData.strings.Common_Delete,
                                color: .destructive,
                                action: { [weak actionSheet, weak self] in
                                    self?.context.account.postbox.delete(folderWithId: peerId)
                        ...
                        (self.navigationController as? NavigationController)?
                            .pushViewController(
                                FolderController(
                                    ...
                                    folderId: -peerId.id,
                                    factory: FolderModuleFactoryImpl.default
                                )
                        )
                    } else {
            ...
            if let strongSelf = self {
                if let navigationController = strongSelf.navigationController as? NavigationController {
                    ...
                    let chatListController = ChatListControllerImpl(
                        ...
                        channelControllerProvider: strongSelf.channelControllerProvider
                    )
                    // MARK: -
    ...
    @objc private func composePressed() {
        if case .unread = chatListMode {
            return context.account.postbox.readAllIncomingMessages()
        }
        ...
        switch chatListMode {
            case .folders:
                let selectionController = ChatListSelectionController(context: context, options: [])

                createFolderActionDisposable.set(
                    (selectionController.result |> deliverOnMainQueue)
                        .start(next: { [context] selectedPeers in
                            ...
                            let createFolder = createFolderController(context: context, peerIds: peerIds)
            ...
            case .filter(.channels):
                controller = channelControllerProvider?(context)
            default:
                break
        }
        
        let targetController = controller ?? context.sharedContext.makeComposeController(context: context)
        
        (self.navigationController as? NavigationController)?.replaceAllButRootController(targetController, animated: true)
    }
    // MARK: -
            ...
            switch item.content {
                ...
                case let .groupReference(groupId, _, _, _, _):
                    ...
                    let chatListController = ChatListControllerImpl(
                        ...
                        controlsHistoryPreload: false,
                        enableDebugActions: false,
                        channelControllerProvider: channelControllerProvider
                    )
                    // MARK: -
...
extension ChatListControllerImpl {
    ...
    var statusBarHeight: CGFloat {
        return validLayout?.statusBarHeight ?? 0.0
    }
    
    private func setup() {
        setupTabBarTapHandler()
        setupUnreadCategoriesCallback()
    }
    
    private func setupUnreadCategoriesCallback() {
        context.account.postbox.setUnreadCategoriesCallback { [weak self] unreadCategories in
            ...
            DispatchQueue.main.async {
                self?.chatListDisplayNode.tabBarView?.setMarks(for: .init(markedTabs))
            }
        }
    }
    
    private func setupTabBarTapHandler() {
        chatListDisplayNode.tabBarView?.tapHandler = { [weak self] in
...
extension ChatListControllerImpl {
    func updateRightBarButton() {
        ...
        navigationItem.accessibilityLabel = accessebilityTitle
    }
    
    private func getRightBarButtonIconAndAccessebilityTitle() -> (icon: UIImage?, accessebilityTitle: String) {
        let accentColor = presentationData.theme.rootController.navigationBar.accentTextColor
        var icon: UIImage?
        var accessebilityTitle: String
        
        switch chatListMode {
        case .folders:
            icon = IMeImageAsset.with(.addIcon(accentColor))
            accessebilityTitle = "Folders"
        case .unread:
            icon = IMeImageAsset.with(.readAllMessageIcon(accentColor))
            accessebilityTitle = "Read all"
        default:
            icon = IMeImageAsset.with(.composeIcon(accentColor))
            accessebilityTitle = "Compose"
        }
        
        return (icon: icon, accessebilityTitle: accessebilityTitle)
    }
    
    private func setupEmptyListPlaceholder() {
        chatListDisplayNode.placeholderType = placeholderType

        DispatchQueue.main.async { [weak self] in
            guard let presentationData = self?.presentationData else { return }
            self?.chatListDisplayNode.updatePresentationData(presentationData)
        }
    }
}
