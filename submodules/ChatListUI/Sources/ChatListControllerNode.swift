...
private struct TestItem: Comparable, MergeLists.Identifiable {
    var value: Int
    var version: Int
...
final class ChatListControllerNode: ASDisplayNode {
    ...
    // MARK: - iMe
    private var tabBarNode: ASDisplayNode? = nil
    
    var tabBarView: TabBarView? {
        return tabBarNode?.view as? TabBarView
    }

    // Initial value is a magical number. Without it first scroll
    // will result top tab bar jerking. Why? I don't give a fuck.
    private var previousContentOffset: CGFloat = 10.0
    ...
    init(
        ...
        filter: ChatListNodePeersFilter? = nil,
        showsFilterTabs: Bool = false,
        placeholderType: PlaceholderType,
        setupChatListModeHandler: @escaping SetupChatListModeCallback
    ) {
    // MARK: -
        ...
        self.chatListNode = ChatListNode(
            ...
            mode: filter
                .map { .peers(filter: $0, showAsChatList: true) }
                .or(.chatList),
            ...
            switch isEmptyState {
                case .empty(false):
                    if case .group = strongSelf.groupId {
                        ...
                        // MARK: - iMe
                        let chatListEmptyNode = ChatListEmptyNode(
                            theme: strongSelf.presentationData.theme,
                            strings: strongSelf.presentationData.strings,
                            placeholderType: strongSelf.placeholderType
                        )
                        // MARK: -
    ...
    // MARK: - iMe
    private func setupTabBarNode() {
        let node = ASDisplayNode { TabBarView(theme: self.presentationData.theme) }
        tabBarNode = node
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, visualNavigationHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        // MARK: - iMe
        self.chatListNode.visualInsets = UIEdgeInsets(top: visualNavigationHeight + TabBarView.height, left: 0.0, bottom: 0.0, right: 0.0)

        updateTabBarLayout(
            with: layout,
            offset: visualNavigationHeight,
...
private extension ChatListControllerNode {

    var statusBarHeight: CGFloat {
        return containerLayout?.0.statusBarHeight ?? 20.0
    }

    var collapsedNavbarHeight: CGFloat {
        return navigationBar
            .and(navigationBar?.contentNode?.frame.height)
            .map { $0.contentHeight - $1 }
            .or(44.0)
    }

    /// The inset of the tab bar when search field is collapsed.
    var regularTabBarViewTopInset: CGFloat {
        return statusBarHeight + collapsedNavbarHeight
    }

    func setupScrollCallbacks() {
        // NOTE: This callback handles the case when the tab bar
        // is either almost revealed or almost hidden by user's
        // dragging action and activated when the user stops it.
        // It snaps tab bar in place depending on whether it's closer
        // to hidden or revealed state.
        //
        // P.S. Please, make sure you fully understand this code
        // prior making any changes.
        
        chatListNode.didEndScroll = { [unowned self] in
            guard let (containerLayout, navigationHeight, visualNavigationHeight) = self.containerLayout else {
                return
            }

            guard
                self.currentTabBarViewOffset != 0.0,
                self.currentTabBarViewOffset != TabBarView.height,
                self.tabBarNode?.frame.minY != visualNavigationHeight,
                self.tabBarNode?.frame.minY != visualNavigationHeight - TabBarView.height
            else { return }

            if visualNavigationHeight == navigationHeight && self.previousContentOffset <= 0.0
                || self.previousContentOffset <= TabBarView.height * 2
                || self.currentTabBarViewOffset >= -TabBarView.height / 2
            {
                self.currentTabBarViewOffset = 0
            } else {
                self.currentTabBarViewOffset = -TabBarView.height
            }
            ...
            self.updateTabBarLayout(
                ...
                offset: visualNavigationHeight + self.currentTabBarViewOffset,
                transition: .animated(duration: 0.1, curve: .easeInOut)
            )
        }

        // NOTE: This callback handled the scrolling. It changes
        // filter tab bar offset so it hides and reveals naturaly.
        chatListNode.didScroll = { [unowned self] contentOffset, isLeap in
            // NOTE: The strange thing about Telegram's custom scroll view
            // is that content offset is not always valid. Somehow
            // after a certain point it takes values of 10 000,
            // then 20 000, and so on. Thus in order to prevent
            // leap of our filter tab bar we need to mark such moments,
            // so the leap is ignored. `isLeap` indicates such a leap.
            //
            // P.S. Some how Telegram itself is able to track right
            // content offset, but I couldn't figure it out. Change this
            // code only if it's really necessary.
            guard
                let (layout, _, visualNavigationHeight) = self.containerLayout,
                visualNavigationHeight <= self.regularTabBarViewTopInset
            else { return }

            let newOffset = contentOffset - TabBarView.height
            guard newOffset > 0 else {
                self.previousContentOffset = 0.0
                return
            }
            ...
            guard !isLeap else { return }

            if self.currentTabBarViewOffset <= 0 && change < 0 {
                self.currentTabBarViewOffset = min(0.0, self.currentTabBarViewOffset - change)
            } else if self.currentTabBarViewOffset > -TabBarView.height && change > 0 {
                self.currentTabBarViewOffset = max(-TabBarView.height, self.currentTabBarViewOffset - change)
            }
    ...
    private func updateTabBarLayout(
        with containerlayout: ContainerViewLayout,
        offset: CGFloat,
        transition: ContainedViewLayoutTransition
    ) {
        tabBarNode.apply {
            transition.updateFrameAdditive(
                node: $0,
                frame: CGRect(
                    origin: CGPoint(x: 0.0, y: offset),
                    size: CGSize(width: containerlayout.size.width, height: TabBarView.height)
                ),
                force: true
            )
        }
