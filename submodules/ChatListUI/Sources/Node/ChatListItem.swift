...
private func revealOptions(strings: PresentationStrings, theme: PresentationTheme, isPinned: Bool, isMuted: Bool?, groupId: PeerGroupId, peerId: PeerId, accountPeerId: PeerId, canDelete: Bool, isEditing: Bool) -> [ItemListRevealOption] {
    ...
    // MARK: - iMe
    if !isEditing && !peerId.isFolderId {
    // MARK: -
                ...
                case let .peer(_, renderedPeer, _, _, presence, _ ,_ ,_, _, _):
                    ...
                    if item.enableContextActions && !isAd {
                        ...
                        if case let .chat(itemPeer) = contentPeer {
                            // MARK: - iMe
                            peerLeftRevealOptions = leftRevealOptions(
                                strings: item.presentationData.strings,
                                theme: item.presentationData.theme,
                                isUnread: unreadCount.unread,
                                isEditing: item.editing,
                                isPinned: isPinned,
                                isSavedMessages: itemPeer.peerId == item.context.account.peerId || itemPeer.peerId.isFolderId,
                                groupId: item.peerGroupId
                            )
                            // MARK: -
                        } else {
...
// MARK: - iMe
extension ChatListItemNode: ChatListItemNodeType {
    var chatListItemType: ChatListItemType? {
        return item
    }
}

extension ChatListItem: ChatListItemType {
    public var iMeContent: IMEChatListItemContent {
        switch content {
        case let .peer(_, peer, _, _, _, _, _, _, _, _):
            return .peer(peer)
        case let .groupReference(groupId, _, _, _, _):
