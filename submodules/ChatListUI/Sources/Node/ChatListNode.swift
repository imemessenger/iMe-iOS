        ...
        switch entry.entry {
            case let .PeerEntry(index, presentationData, message, combinedReadState, notificationSettings, embeddedState, peer, presence, summaryInfo, editing, hasActiveRevealControls, selected, inputActivities, isAd):
                ...
                switch mode {
                    ...
                    // MARK: - iMe
                    case let .peers(filter, showAsChatList):
                    // MARK: -
                        ...
                        // MARK: - iMe
                        let listViewItem: ListViewItem
                        if showAsChatList {
                            listViewItem = ChatListItem(
                        ...
                        return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: listViewItem, directionHint: entry.directionHint)
                    // MARK: -
                }
...
public enum ChatListNodeEmptyState: Equatable {
    ...
    // MARK: - iMe
    var iMeEmptyState: ImeChatListNodeEmptyState {
        switch self {
        case let .notEmpty(containsChats: contains):
            return .notEmpty(containsChats: contains)
        case let .empty(isLoading: isLoading):
...
public final class ChatListNode: ListView {
    ...
    // MARK: - iMe
    public var iMEisEmptyUpdated: ((ImeChatListNodeEmptyState) -> Void)?
    // MARK: -
    ...
    public init(
        ...
        setupChatListModeHandler: SetupChatListModeCallback? = nil,
        ...
        |> mapToQueue { (hideArchivedFolderByDefault, displayArchiveIntro, savedMessagesPeer, update, state) -> Signal<ChatListNodeListViewTransition, NoError> in
            ...
            // MARK: - iMe
            if case let .peers(_, showAsChatList) = mode, !showAsChatList {
            // MARK: -
                searchMode = true
            }
            ...
            let completion: (ListViewDisplayedItemRange) -> Void = { [weak self] visibleRange in
                if let strongSelf = self {
                    ...
                    if strongSelf.currentIsEmptyState != isEmptyState {
                        ...
                        // MARK: - iMe
                        strongSelf.iMEisEmptyUpdated?(isEmptyState.iMeEmptyState)
                        // MARK: -
                    }
...
// MARK: - iMe
extension ChatListNode: ChatListNodeInterface {
    ...
    func snapToTop() {
        let location: ChatListNodeLocation = .scroll(
            index: .absoluteUpperBound,
            sourceIndex: .absoluteLowerBound,
            scrollPosition: .top(10.0),
            animated: false
        )
        self.setChatListLocation(location)
    }
