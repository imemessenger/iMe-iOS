...
// MARK: -
    switch location {
        case let .initial(count):
            ...
            // MARK: - iMe
            signal = account.viewTracker.tailChatListView(groupId: groupId, count: count, applyFiltration: true, setupChatListModeHandler: setupChatListModeHandler)
            // MARK: -
            return signal
        ...
        case let .navigation(index):
            var first = true
            // MARK: - iMe
            return account.viewTracker.aroundChatListView(groupId: groupId, index: index, count: 80, applyFiltration: true, setupChatListModeHandler: setupChatListModeHandler)
            // MARK: -
            |> map { view, updateType -> ChatListNodeViewUpdate in
