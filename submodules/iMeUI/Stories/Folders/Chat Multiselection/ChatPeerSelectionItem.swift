//
//  ChatPeerSelectionItem.swift
//  iMeUI
//
//  Created by Oleksandr Shynkarenko on 10/11/19.
...
enum ChatPeerSelectionItemStatus {
    case none
...
enum ChatPeerSelectionItemSelection: Equatable {
    case none
    case selectable(selected: Bool)

    static func ==(lhs: ChatPeerSelectionItemSelection, rhs: ChatPeerSelectionItemSelection) -> Bool {
        switch lhs {
        case .none:
            if case .none = rhs {
                return true
...
struct ChatPeerSelectionItemEditing: Equatable {
    let editable: Bool
    let editing: Bool
    let revealed: Bool

    static func ==(lhs: ChatPeerSelectionItemEditing, rhs: ChatPeerSelectionItemEditing) -> Bool {
...
enum ChatPeerSelectionItemPeerMode {
    case generalSearch
    case peer
}

enum ChatPeerSelectionItemBadgeType {
    case active
    case inactive
}

struct ChatPeerSelectionItemBadge {
    let count: Int32
    let type: ChatPeerSelectionItemBadgeType
}

enum ChatPeerSelectionItemPeer: Equatable {
    case peer(peer: Peer?, chatPeer: Peer?)
    case deviceContact(stableId: DeviceContactStableId, contact: DeviceContactBasicData)

    static func ==(lhs: ChatPeerSelectionItemPeer, rhs: ChatPeerSelectionItemPeer) -> Bool {
        switch lhs {
...
class ChatPeerSelectionItem: ListViewItem {
    let theme: PresentationTheme
    let strings: PresentationStrings
    let sortOrder: PresentationPersonNameOrder
    let displayOrder: PresentationPersonNameOrder
    let account: Account
    let peerMode: ChatPeerSelectionItemPeerMode
    let peer: ChatPeerSelectionItemPeer
    let status: ChatPeerSelectionItemStatus
    let badge: ChatPeerSelectionItemBadge?
    let enabled: Bool
    let selection: ChatPeerSelectionItemSelection
    let editing: ChatPeerSelectionItemEditing
    let action: (ChatPeerSelectionItemPeer) -> Void
    ...
    init(theme: PresentationTheme, strings: PresentationStrings, sortOrder: PresentationPersonNameOrder, displayOrder: PresentationPersonNameOrder, account: Account, peerMode: ChatPeerSelectionItemPeerMode, peer: ChatPeerSelectionItemPeer, status: ChatPeerSelectionItemStatus, badge: ChatPeerSelectionItemBadge? = nil, enabled: Bool, selection: ChatPeerSelectionItemSelection, editing: ChatPeerSelectionItemEditing, index: PeerNameIndex?, header: ListViewItemHeader?, action: @escaping (ChatPeerSelectionItemPeer) -> Void, setPeerIdWithRevealedOptions: ((PeerId?, PeerId?) -> Void)? = nil, deletePeer: ((PeerId) -> Void)? = nil) {
        self.theme = theme
    ...
    func nodeConfiguredForParams(
        ...
        async {
            let node = ChatPeerSelectionItemNode()
            let makeLayout = node.asyncLayout()
            let (first, last, firstWithHeader) = ChatPeerSelectionItem.mergeType(
                item: self,
    ...
    func updateNode(
        ...
        Queue.mainQueue().async {
            if let nodeValue = node() as? ChatPeerSelectionItemNode {
                let layout = nodeValue.asyncLayout()
                async {
                    let (first, last, firstWithHeader) = ChatPeerSelectionItem.mergeType(item: self, previousItem: previousItem, nextItem: nextItem)
    ...
    static func mergeType(item: ChatPeerSelectionItem, previousItem: ListViewItem?, nextItem: ListViewItem?) -> (first: Bool, last: Bool, firstWithHeader: Bool) {
        var first = false
        var last = false
        var firstWithHeader = false
        if let previousItem = previousItem {
            if let header = item.header {
                if let previousItem = previousItem as? ChatPeerSelectionItem {
        ...
        if let nextItem = nextItem {
            if let header = item.header {
                if let nextItem = nextItem as? ChatPeerSelectionItem {
...
class ChatPeerSelectionItemNode: ItemListRevealOptionsItemNode {
    ...
    private var layoutParams: (ChatPeerSelectionItem, ListViewItemLayoutParams, Bool, Bool, Bool)?
    var chatPeer: Peer? {
    ...
    private var item: ChatPeerSelectionItem? {
    ...
    override func layoutForParams(_ params: ListViewItemLayoutParams, item: ListViewItem, previousItem: ListViewItem?, nextItem: ListViewItem?) {
        if let (item, _, _, _, _) = self.layoutParams {
            let (first, last, firstWithHeader) = ChatPeerSelectionItem.mergeType(item: item, previousItem: previousItem, nextItem: nextItem)
    ...
    func asyncLayout() -> (_ item: ChatPeerSelectionItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool) -> (ListViewItemNodeLayout, () -> (Signal<Void, NoError>?, (Bool) -> Void)) {
                ...
                if let strongSelf = self {
                    ...
                    case let .peer(peer, _):
                        if let peer = peer {
                            ...
                            strongSelf.avatarNode.setPeer(account: item.account, theme: item.theme, peer: peer, overrideImage: overrideImage)
                        }
                    case let .deviceContact(_, contact):
                        let letters: [String]
                        if !contact.firstName.isEmpty && !contact.lastName.isEmpty {
